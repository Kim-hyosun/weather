		...
		<div className="mapdata">
			{
				Currentdata.map((item, idx) => <div key={item.data.city.id} className={'wt' + idx}>
					...
					<div className="temp">{item.data.list[0].main.temp_min ? item.data.list[0].main.temp_min : "loading"}/{item.data.list[0].main.temp_max ? item.data.list[0].main.temp_max : "loading"}℃ <br /> (체감{item.data.list[0].main.feels_like ? item.data.list[0].main.feels_like : "loading"}℃)</div>
				</div>
				)
			}
		</div >

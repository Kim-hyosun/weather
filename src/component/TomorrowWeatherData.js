		...
		<div className="mapdata">
			{
				data.map((item, idx) => <div key={item.data.city.id} className={'wt' + idx}>
					...
					<div className="temp">{item.data.list[12].main.temp ? item.data.list[12].main.temp : "loading"}℃</div>
					<div style={{whiteSpace:'nowrap'}}>(체감{item.data.list[12].main.feels_like ? item.data.list[12].main.feels_like : "loading"}℃)</div>
				</div>
				)
			}
		</div >
